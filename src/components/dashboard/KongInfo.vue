    ...
    methods: {
      ...
      loadStatus () {
        ...
        if (localStorage.isLoad) {
          ...
          this.$axios.get('http://localhost:9090/api/v1/query?query=sum(kong_nginx_http_current_connections{state="total", instance=~"' + localStorage.selectedVal + '"})')
            .then((response) => {
          ...
          this.$axios.get('http://localhost:9090/api/v1/query?query=sum(kong_nginx_http_current_connections{state="handled", instance=~"' + localStorage.selectedVal + '"})')
            .then((response) => {
              // console.log(response)
              let kongStatus = response.data.data.result[0].value[1]
          ...
          this.$axios.get('http://localhost:9090/api/v1/query?query=sum(kong_nginx_http_current_connections{state="accepted", instance=~"' + localStorage.selectedVal + '"})')
            .then((response) => {
              // console.log(response)
              let kongStatus = response.data.data.result[0].value[1]
