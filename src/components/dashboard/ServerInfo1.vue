  ...
  export default {
    ...
    methods: {
      async loadServerInfo () {
        ...
        let _this = this
        // let serverInfo2 = []
        await this.$axios.get('/query?query=node_uname_info')
          .then((response) => {
            // _this.serverInfo = []
      ...
      loadCpuTotal (job) {
        let url = '/query?query=count(node_cpu_seconds_total{job=~"' + job + '"' + ",mode='system'}) by (instance)"
        this.$axios.get(url)
          .then((response) => {
            for (var i = 0; i < this.serverInfo.length; i++) {
      ...
      loadBootTime (job) {
        this.$axios.get('/query?query=sum(time() - node_boot_time_seconds{job=~"' + job + '"})by(instance)')
          .then((resp) => {
            // bootTime
            for (var i = 0; i < this.serverInfo.length; i++) {
      ...
      loadFilesystem (job) {
        let url = '/query?query=max((node_filesystem_size_bytes{job=~"' + job + '",fstype=~"ext.?|xfs"}-node_filesystem_free_bytes{job=~"' + job + '",fstype=~"ext.?|xfs"})*100/(node_filesystem_avail_bytes{job=~"' + job + '",fstype=~"ext.?|xfs"}%2B(node_filesystem_size_bytes{job=~"' + job + '",fstype=~"ext.?|xfs"}-node_filesystem_free_bytes{job=~"' + job + '",fstype=~"ext.?|xfs"})))by(instance)'
        this.$axios.get(url)
          .then((resp) => {
            for (var i = 0; i < this.serverInfo.length; i++) {
      ...
      loadCpuRate (job) {
        let url = '/query?query=(1 - avg(irate(node_cpu_seconds_total{job=~"' + job + '",mode="idle"}[5m])) by (instance)) * 100'
        this.$axios.get(url)
          .then((resp) => {
            // console.log(resp.data.data.result[0].value[1])
            for (var i = 0; i < this.serverInfo.length; i++) {
      ...
      loadMemoryRate (job) {
        let url = '/query?query=(1 - (node_memory_MemAvailable_bytes{job=~"' + job + '"} / (node_memory_MemTotal_bytes{job=~"' + job + '"})))* 100'
        this.$axios.get(url)
          .then((resp) => {
            // console.log(resp.data.data.result)
            for (var i = 0; i < this.serverInfo.length; i++) {
      ...
      loadDownBandwidth (job) {
        let url = '/query?query=max(irate(node_network_receive_bytes_total{job=~"' + job + '"}[5m])*8) by (instance)'
        this.$axios.get(url)
          .then((resp) => {
            // console.log(resp.data.data.result)
            for (var i = 0; i < this.serverInfo.length; i++) {
      ...
      loadUpBandwidth (job) {
        let url = '/query?query=max(irate(node_network_transmit_bytes_total{job=~"' + job + '"}[5m])*8) by (instance)'
        this.$axios.get(url)
          .then((resp) => {
            // console.log(resp.data.data.result)
            for (var i = 0; i < this.serverInfo.length; i++) {
